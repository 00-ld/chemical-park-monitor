...
当前仓库保留现有技术栈与目录命名：前端在 `Manage/`，Java 后端在 `Back/`，Python 算法服务在 `python/`，数据库台账在 `db/`，部署配置在 `deploy/`。后续若迁移到更规范目录，应另开计划执行。
...
| 模块 | 目录 | 技术 |
| --- | --- | --- |
| 前端 | `Manage/` | Vue 3、TypeScript、Vite、Element Plus、Pinia、ECharts、Canvas |
| 后端 | `Back/` | Spring Boot 3.4、JDK 21、MyBatis、MySQL、JWT |
| 算法服务 | `python/` | FastAPI、NumPy、SciPy、PyTorch、Ultralytics YOLO11 |
| 三维/数字孪生 | `Manage/src/views/screen/` | SuperMap iPortal 数字大屏 URL 嵌入 |
...
.
  ...
  Manage/            Vue 3 前端管理系统
  python/            Python 算法服务与扩散/溯源/路径规划算法
  GasModelTest/      气体模型可复现测试数据与验证脚本
...
`GasModelTest/` 当前数据是合成解析数据，用于回归验证，不得写成现场实测数据。真实/权威数据来源台账见 [docs/dataset-sources.md](docs/dataset-sources.md)。
