...
| 扩散修正系数标定 | 已补充 | `calibration.py` 和 `test_calibration.py`，用于基于观测数据客观选择 `diffusionCorrectionK` |
| Prairie Grass 真实数据验证 | 暂不提交 | 依赖未入库大数据文件，需先完成来源、许可和字段说明 |
| 大型真实数据集 | 暂不提交 | 仅记录来源台账，必要时通过外部存储或下载脚本复现 |

`diffusionCorrectionK` 标定只负责在给定观测数据和候选 K 网格上计算 FB、NMSE、FAC2 等指标，并选择 NMSE 最小的参数。它不改变现有前端接口中 `diffusionCorrectionK=1` 的改进模型基线语义，也不能替代真实数据集验证。
